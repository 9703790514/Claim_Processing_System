...
public class DocumentController {
	...
	public ResponseEntity<DocumentsEO> getById(@PathVariable Integer _id) {
		return docSer.findById(_id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}
	...
	public ResponseEntity<DocumentsEO> updateDocument(@PathVariable Integer _id, @RequestBody DocumentsEO doc) {
		return docSer.update(_id, doc).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}
