...
function ClaimsTable({
    ...
    const showFieldDoctorColumn = selectedStatus === 'raised';
    const getDoctorName = (id) =>
        fieldDoctors.find(doc => doc.id === id)?.name || "Not Assigned";
                    ...
                    <Table stickyHeader aria-label="claims table">
                        <TableHead className="MuiTableHead-root">
                            <TableRow>
                                ...
                                {showFieldDoctorColumn && <TableCell>Field Doctor Name</TableCell>}
                            ...
                            {filteredClaims.map((claim) => (
                                <TableRow key={claim.id} hover className="MuiTableRow-root">
                                        ...
                                        <TableCell sx={{ textAlign: 'center' }}>
                                            {claim.field_doctor_id ? (
                                                <span style={{ color: 'green', fontWeight: 600 }}>
                                                    <CheckCircleOutlineIcon color="success" fontSize="small" /> Yes
                                                </span>
                                            ) : (
                                                <span style={{ color: 'red', fontWeight: 600 }}>
                                                    <HighlightOffIcon color="error" fontSize="small" /> No
                                                </span>
                                            )}
                                        </TableCell>
                                    )}
                                    {showFieldDoctorColumn && (
                                        <TableCell>
                                            {claim.field_doctor_id
                                                ? getDoctorName(claim.field_doctor_id)
                                                : "Not Assigned"}
                                        </TableCell>
                                    )}
