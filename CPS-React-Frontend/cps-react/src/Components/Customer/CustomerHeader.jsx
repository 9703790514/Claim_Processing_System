...
import ProfileMenu from '../CustProfile';
...
export default function CustomerHeader() {
  ...
  const patientId = "policyholder_id_DEF";
  const policyHolderId = "policyholder_id_DEF";
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      case 'home':
        return (
          <>
            <RecentClaims patientId={patientId} onClaimSelect={setSelectedClaim} />
            {!selectedClaim && <PolicyDetails policyHolderId={policyHolderId} onClaimSelect={setSelectedClaim}/>}
          </>
        );
      case 'reimbursement':
        return <ReimbursementComponent />; // Assuming ReimbursementComponent doesn't need onNavigate here
      case 'viewHospitals':
        return <ViewHospitals />; // Assuming ViewHospitals doesn't need onNavigate here
      default:
        return (
          <>
           <RecentClaims patientId={patientId} onClaimSelect={setSelectedClaim} />
    ...
    <>
      <header style={styles.header}>
        <div style={styles.logoAndNavContainer}>
          ...
          <button
            onClick={() => handleNavigate('viewHospitals')}
            style={{ ...styles.navButton, ...(currentPage === 'viewHospitals' ? styles.navButtonActive : {}) }}
          >
            View Hospitals
          </button>
        </div>
        ...
        <div style={styles.profileContainer}>
          ...
          {showProfile && (
            <div style={styles.profileDropdown}>
              <ProfileMenu hospitalId={hospitalId}/>
            </div>
          )}
        </div>
      </header>
